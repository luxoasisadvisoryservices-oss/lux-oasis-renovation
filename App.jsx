...
// ─────────────────────────────────────────────────────────────────
// TurnkeySection renders three blocks:
//   A) Turnkey outcome selector (STR / Furnished Rental / Flip / Investment)
//   B) Delivery breakdown (what's included in a full project)
//   C) "We understand your world" STR knowledge pillars
//
// It is placed below the "Why Clients Work With Us" section.
// To move it: cut <TurnkeySection /> and paste it anywhere in the return.
// To edit copy: open src/data/content.js and update:
//   → turnkeyOutcomes / turnkeyDelivery / strPillars
// ─────────────────────────────────────────────────────────────────
...
// ─── Image placeholder with graceful fallback ───
...
// ─── Video block with fallback ───
function VideoBlock({ src, poster, label }) {
  const [hasFile] = useState(false); // set to true once you upload the video
  if (!hasFile) {
    return (
    ...
    <div className="relative group">
      <div className="aspect-[4/3] overflow-hidden bg-stone-100">
        <ProjectImage
          ...
          alt={`${label} — ${showAfter ? "After" : "Before"}`}
      ...
      <div className="absolute top-3 left-3 flex gap-1">
        <button
          ...
          className={`px-3 py-1 text-xs font-medium tracking-widest uppercase transition-all ${!showAfter ? "bg-stone-900 text-stone-50" : "bg-stone-900/50 text-stone-300 hover:bg-stone-900/70"}`}
        >
          Before
        </button>
        <button
          onClick={() => setShowAfter(true)}
          className={`px-3 py-1 text-xs font-medium tracking-widest uppercase transition-all ${showAfter ? "bg-amber-800 text-stone-50" : "bg-stone-900/50 text-stone-300 hover:bg-stone-900/70"}`}
        >
          After
        </button>
      </div>
  ...
  return (
    <div className={`border-b border-stone-200 transition-colors ${open ? "" : "hover:border-stone-400"}`}>
      <button
        ...
        aria-expanded={open}
      >
        <span className={`text-base font-medium transition-colors ${open ? "text-amber-800" : "text-stone-800 group-hover:text-amber-800"}`}>
          {question}
        </span>
  ...
  return (
    <div className="font-body text-stone-800 bg-stone-50 overflow-x-hidden">
      ...
      <section className="relative min-h-[92vh] flex items-center overflow-hidden bg-stone-900">
        {/* Background image — REPLACE src with /images/greens/hero.jpg or your chosen hero shot */}
        <div className="absolute inset-0">
          <ProjectImage
            src="/images/greens/hero.jpg"
            alt="Lux Oasis Renovation — Dubai Property Transformation"
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 py-24">
          <div className="max-w-2xl">
            ...
            <h1 className="font-display text-4xl md:text-5xl lg:text-6xl xl:text-7xl text-stone-50 leading-[1.05] mb-6">
              Properties
              <br />
              <span className="text-amber-200">Transformed</span>
              <br />
              with Purpose.
            </h1>
            <p className="text-stone-300 text-lg md:text-xl leading-relaxed mb-10 max-w-xl">
              Strategic renovation coordination for Dubai's holiday home owners,
              investors, and landlords. We manage the entire transformation —
              so you receive the outcome, not the complexity.
            </p>
        ...
        <div className="absolute right-0 top-0 bottom-0 w-px bg-gradient-to-b from-transparent via-amber-700/30 to-transparent hidden lg:block" />
      </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">
            <div>
              ...
              <p className="text-stone-400 leading-relaxed mb-8 text-lg">
                Lux Oasis Renovation is a project management and coordination
                practice. We define the vision, identify the right specialists, manage
                execution quality, and ensure the finished property meets its intended
                market position.
              </p>
              <p className="text-stone-400 leading-relaxed text-base">
                We coordinate the vision, the execution flow, and the delivery
                process through trusted third-party specialists. We do not operate
                as a construction contractor, engineering consultancy, or brokerage.
              </p>
            </div>
                ...
                <ul className="space-y-3">
                  {[
                    ...
                    <li key={item} className="flex items-start gap-3 text-stone-300 text-sm">
                      <svg className="mt-0.5 flex-shrink-0 text-amber-500" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                ...
                <ul className="space-y-3">
                  {[
                    ...
                    <li key={item} className="flex items-start gap-3 text-stone-500 text-sm">
                      <svg className="mt-0.5 flex-shrink-0 text-stone-600" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                        <path d="M18 6 6 18M6 6l12 12" />
                      </svg>
                      {item}
                    </li>
            ...
            {[
              {
                ...
                desc: "Units that are designed and styled for their target market — whether Airbnb guest, long-term tenant, or buyer.",
              },
              {
                number: "02",
                title: "STR-Optimised Decisions",
                desc: "Every upgrade is evaluated through the lens of occupancy, nightly rate, and guest experience — not just aesthetics.",
              },
              {
                number: "03",
                title: "Investor-Smart Spending",
                desc: "We help clients avoid over-building and under-building. Renovation scope is calibrated to financial logic, not personal preference.",
              },
              {
                number: "04",
                title: "Fully Managed Experience",
                desc: "You are involved in decisions, not logistics. We manage the execution layer so your energy stays at the strategic level.",
              },
            ].map((item, i) => {
              const [ref, inView] = useInView();
              return (
      ...
      {/* ── TURNKEY POSITIONING — 3 blocks: outcomes / delivery / STR understanding ── */}
      {/* Edit copy in src/data/content.js → turnkeyOutcomes, turnkeyDelivery, strPillars */}
      <TurnkeySection />
      ...
      <section id="projects" className="py-24 md:py-32 bg-stone-50">
        <div className="max-w-7xl mx-auto px-6">
          <SectionHeading
            ...
            subtitle="A selection of transformation work — each one approached with the same strategic discipline: upgrade with intent, invest with logic."
          />
                ...
                >
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-4 mb-8 pb-6 border-b border-stone-200">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {project.tags.map((tag) => (
                        <span key={tag} className="px-3 py-1 text-xs bg-stone-100 text-stone-600 border border-stone-200 tracking-wide">
                          {tag}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className={`grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-start ${isEven ? "" : "lg:flex-row-reverse"}`}>
                    ...
                    <div className={isEven ? "" : "lg:order-2"}>
                      ...
                      {project.video && (
                        <div className="mt-8">
                          ...
                          {/* ─── Upload your video to /public{project.video} ─── */}
                    ...
                    <div className={`space-y-4 ${isEven ? "" : "lg:order-1"}`}>
                      {/* ─── Hero project image — replace path in content.js ─── */}
                      <ProjectImage
                      ...
                      {project.images.length > 0 && (
                        <div className="grid grid-cols-3 gap-3">
                          {project.images.slice(0, 3).map((img) => (
                            <div key={img.src}>
                              {/* ─── Replace image paths in src/data/content.js ─── */}
                              <ProjectImage
                                src={img.src}
                                alt={img.label}
      ...
      {/* ── 7. TRANSFORMATION THINKING ── */}
      <section className="py-24 md:py-32 bg-stone-900 overflow-hidden">
        <div className="max-w-7xl mx-auto px-6">
          <SectionHeading
            eyebrow="Our Approach"
            title="Transformation is a discipline, not an impulse."
            subtitle="Every project we take on follows the same structured logic — from assessment through to final performance-ready handover."
            light
          />
              ...
              {[
                ...
                return (
                  <div
                    ...
                    ref={ref}
                    style={{ transitionDelay: `${i * 80}ms` }}
                    className={`relative flex items-center gap-8 transition-all duration-600 ${inView ? "opacity-100 translate-x-0" : isLeft ? "opacity-0 -translate-x-8" : "opacity-0 translate-x-8"} flex-col sm:flex-row ${i % 2 !== 0 ? "sm:flex-row-reverse" : ""}`}
                  >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {processSteps.map((step, i) => {
              ...
              return (
                ...
                >
                  <div className="font-display text-6xl text-stone-100 font-light absolute top-4 right-6 select-none">
                    {step.step}
                  </div>
                  <div className="relative">
                  ...
                  {i < processSteps.length - 1 && (
                    <div className="absolute -right-3 top-1/2 -translate-y-1/2 w-6 h-px bg-amber-300 hidden lg:block" />
                  )}
                </div>
              );
            })}
      ...
      <section id="ecosystem" className="py-24 md:py-32 bg-amber-900 text-stone-50">
        <div className="max-w-7xl mx-auto px-6">
              ...
              return (
                <div
                  ...
                  className={`p-6 border transition-all duration-500 ${brand.current
                    ...
                    : "border-amber-800 bg-amber-900/40 hover:border-amber-600 hover:bg-amber-800/30"
          ...
          <div className="mt-12 pt-10 border-t border-amber-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
            ...
            <CTAButton href="#" variant="outlineLight">Visit Main Company →</CTAButton>
          </div>
        </div>
      </section>
      ...
      <section id="contact" className="py-24 md:py-32 bg-stone-900 relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-6 text-center">
          ...
          <h2 className="font-display text-4xl md:text-5xl lg:text-6xl text-stone-50 leading-tight mb-6">
            Your property has more
            <br />
            potential than it's showing.
          </h2>
          <p className="text-stone-400 text-lg leading-relaxed max-w-xl mx-auto mb-12">
            Whether you're preparing a unit for short-term rental, positioning
            it for sale, or simply ready to invest in a stronger product — we're
            here to manage the transformation.
          </p>
          ...
          <div className="mt-14 pt-8 border-t border-stone-800 flex flex-wrap gap-8 justify-center">
            {[
              ...
              { label: "WhatsApp", value: "+971 58 508 9383", href: "https://wa.me/971585089383" },
              ...
              <div key={item.label} className="text-center">
                ...
                {item.href ? (
                  <a href={item.href} className="text-stone-300 text-sm hover:text-amber-300 transition-colors">{item.value}</a>
                ) : (
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-10 mb-10">
            ...
            <div className="md:col-span-1">
              ...
              <p className="text-stone-500 text-sm leading-relaxed max-w-xs">
                Strategic renovation project management for Dubai property owners, investors, and STR operators.
              </p>
            </div>
            ...
            <div>
              ...
              <ul className="space-y-2">
                {["Services", "Projects", "Process", "Ecosystem", "FAQ", "Contact"].map((link) => (
                  <li key={link}>
                    <a href={`#${link.toLowerCase()}`} className="text-stone-500 hover:text-stone-200 text-sm transition-colors">
                      {link}
                    </a>
                  </li>
                ))}
            ...
            <div>
              ...
              <ul className="space-y-3">
                <li>
                  <a href="mailto:info@luxoasisadvisory.com" className="text-stone-500 hover:text-amber-400 text-sm transition-colors break-all">
                    info@luxoasisadvisory.com
                  </a>
                </li>
                <li>
                  <a href="https://wa.me/971585089383" className="flex items-center gap-2 text-stone-500 hover:text-amber-400 text-sm transition-colors">
            ...
            <div>
              ...
              <p className="text-stone-600 text-xs leading-relaxed mb-4">
                A multi-brand platform built around property operations, revenue optimisation, and strategic advisory in Dubai.
              </p>
              <a href="#" className="text-amber-700 hover:text-amber-500 text-xs tracking-wide transition-colors block mb-6">
                Visit Main Company Website →
              </a>
              <p className="text-stone-300 text-xs tracking-widest uppercase mb-3">Legal</p>
              <ul className="space-y-2">
                <li>
                  <button onClick={() => document.getElementById('terms-modal').classList.remove('hidden')} className="text-stone-500 hover:text-stone-200 text-xs transition-colors">
                    Terms & Conditions
                  </button>
                </li>
                <li>
                  <button onClick={() => document.getElementById('privacy-modal').classList.remove('hidden')} className="text-stone-500 hover:text-stone-200 text-xs transition-colors">
                    Privacy Policy
                  </button>
                </li>
              </ul>
